    ...
    fun `server exposes session metadata and creates fake clients over http`() = withHttpClient { http ->
        LocalSessionApiServer.inMemory().use { server ->
            ...
            http.get(server.url("/clients/alice/openapi.json")).let { clientOpenapi ->
                ...
                assertTrue(body.contains("\"x-craftwright-loader-version\":\"none\""))
                assertTrue(body.contains("\"x-craftwright-driver-version\":\"0.1.0-SNAPSHOT\""))
                assertTrue(body.contains("\"x-craftwright-mappings\":\"none\""))
                assertTrue(body.contains("\"x-craftwright-installed-mods-fingerprint\":\"none\""))
                assertTrue(body.contains("\"x-craftwright-registry-fingerprint\":\"none\""))
                assertTrue(body.contains("\"x-craftwright-server-feature-fingerprint\":\"none\""))
                assertTrue(body.contains("\"x-craftwright-permissions-fingerprint\":\"local-fake\""))
                assertTrue(body.contains("\"x-craftwright-runtime-fingerprint\""))
