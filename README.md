...
| Latest and older lanes | Representative older `1.20.6` packaged attach is verified; `latest-release` resolves to `26.2`, fails early without a packaged 26.x driver lane, and the latest official module now launches, self-attaches, and captures generated OpenAPI metadata in an opt-in probe, with gameplay actions still empty |
