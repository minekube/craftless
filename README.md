...
| Latest and older lanes | Representative older `1.20.6` packaged attach is verified; `latest-release` resolves to `26.2`, fails early without a packaged 26.x driver lane, and the latest official module has a non-remap compiled probe jar plus an opt-in launch/self-attach probe harness |
