...
class MinecraftServerJarProvisioner(
    ...
    suspend fun provision(version: String, target: Path): Path {
        require(version.isNotBlank()) { "minecraft server version is required" }
        val manifest = json.decodeFromString<LauncherVersionManifest>(
            http.get(manifestUrl).bodyAsText()
        )
        val versionUrl = manifest.versions
            .firstOrNull { it.id == version }
            ?.url
            ?: error("minecraft server version $version not found in version manifest")
        val versionMetadata = json.decodeFromString<LauncherVersionMetadata>(
            http.get(versionUrl).bodyAsText()
        )
        val serverUrl = versionMetadata.downloads.server?.url
