...
- Root and module build files, version catalogs, imported platforms, and plugin versions.
...
- Kotlin stdlib, kotlinx.coroutines, kotlinx.serialization, Ktor, Netty, SLF4J, Logback, Fabric, and Loom-related families should usually be version-aligned as families, not patched one artifact at a time.
...
- Shading or relocation is rarely the first answer. Reach for it only when classpath isolation is truly required.
...
- Platform import order and multiple constraints can subtly change which family wins even when the declared versions look reasonable.
...
- If a library family is already managed by a shared platform or catalog, prefer returning ownership there over adding more local pins.
...
- Prefer removing an unnecessary explicit version when the shared platform or catalog already manages a compatible one.
