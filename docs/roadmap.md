...
- Cache per-client OpenAPI only by runtime/action fingerprint. The daemon now
  ...
  `If-None-Match`; `craftless clients <id> actions --openapi-cache <dir>` and
  `craftless clients <id> resources --openapi-cache <dir>` now persist the
  live per-client OpenAPI body plus ETag and revalidate it across CLI
  invocations. Broader durable generated-client and CLI cache coverage remains
  future work.
