...
| Current state | All original CL gates are closed. Phases 198-206 add the support matrix, supported-row probe workflow, runtime-target create rejection, game-scoped Fabric Loader compatibility, and structured `UNSUPPORTED_RUNTIME_TARGET` details. Phase 207 improves ambiguous `craftless api` help after session `019f121c` showed agents had to guess `/clients` method selection. Release `v0.3.0` is published. |
